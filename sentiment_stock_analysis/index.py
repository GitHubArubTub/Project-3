...
# ------------------------
def button_sentiment_analisys(news_articles, ticker_search):
    data_list = Gpt().read_news_articles(news_articles, ticker_search)
...
def draw_sentiment_analisys_chart(sentiment_df):
    ...
    if sentiment_counts.size > 0:
        ...
        chart = alt.Chart(data_melted).mark_bar().encode(
            x=alt.X('variable:N', title='Sentiment'),
...
def draw_prediction_analisys_chart(sentiment_df):
    ...
    if sentiment_counts.size > 0:
        ...
        # Add a title to the chart
        chart = chart.properties(title='Sentiment Analysis of Headlines')
        st.altair_chart(chart)


# rate_limit = st.sidebar.number_input("Rate Limit:", value=3, min_value=1, max_value=30)
news_limit = st.sidebar.number_input("News:", value=5, min_value=1, max_value=30)
if st.sidebar.button("Sentiment analysis"):
    try:
        if not ticker_search:
            st.sidebar.error('"Ticker" and "News" are mandatory fields')
        else:
            ...
            # make alalisys using openapi
            sentiment_df = button_sentiment_analisys(news_articles, ticker_search)
            st.dataframe(sentiment_df)
            draw_sentiment_analisys_chart(sentiment_df)
            st.divider()
